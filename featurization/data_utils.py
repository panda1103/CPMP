...
def load_data_from_df(dataset_path, ff, ignoreInterfragInteractions, add_dummy_node=True, one_hot_formal_charge=False, use_data_saving=True):
    ...
    x_all, y_all = load_data_from_smiles(data_x, data_y, ff, ignoreInterfragInteractions, 
...
def load_data_from_smiles(x_smiles, labels, ff, ignoreInterfragInteractions, add_dummy_node=True, one_hot_formal_charge=False):
    ...
    for smiles, label in zip(x_smiles, labels):
        try:
            ...
            try:
                ...
                if ff == 'uff':
                    AllChem.UFFOptimizeMolecule(mol, ignoreInterfragInteractions = ignoreInterfragInteractions)
                else:
                    AllChem.MMFFOptimizeMolecule(mol, ignoreInterfragInteractions = ignoreInterfragInteractions)
...
def mol_collate_func(batch):
    ...
    return [torch.tensor(adjacency_array), torch.tensor(features_array), torch.tensor(distance_array), torch.tensor(labels_array)]
